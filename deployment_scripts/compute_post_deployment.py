...
def ssh_copy_id(host, username, password, priv_key, pub_key):
    ...
    with open(ssh_askpass, 'w') as f:
        f.write(s)
    os.chmod(ssh_askpass, stat.S_IEXEC)

    logging.info('old SSH_ASKPASS: %s' % os.environ.get('SSH_ASKPASS'))
    logging.info('old DISPLAY: %s' % os.environ.get('DISPLAY'))

    os.putenv('SSH_ASKPASS', XS_RSA)
    os.putenv('DISPLAY', ":0")
...
def get_options(astute, astute_section):
    ...
    logging.info('ssh_pub_key: {ssh_pub_key}'.format(**options))
...
if __name__ == '__main__':
    ...
    if astute:
        username, password, install_xapi, priv_key, pub_key \
        ...
        if username and password and endpoints and himn_local:
            ssh_copy_id(HIMN_IP, username, password, priv_key, pub_key)
