...
def enable_dom0_mesh_bridge(himn, username, bridge, mesh_info):
    filename = 'ifcfg-%' % bridge
    with open(filename, "w") as cfg:
        cfg.writelines('DEVICE=%s' % bridge)
        cfg.writelines('ONBOOT=yes')
        cfg.writelines('NAME=%s' % bridge)
        cfg.writelines('BOOTPROTO=static')
        cfg.writelines('IPADDR=%s' % mesh_info['ipaddr'])
        cfg.writelines('NETMASK=%s' % mesh_info['netmask'])
        cfg.writelines('NETWORK=%s' % mesh_info['network'])
        cfg.writelines('BROADCAST=%s' % mesh_info['broadcast'])

    dom0_path = '/etc/sysconfig/network-scripts'
    utils.scp(himn, username, dom0_path, filename)
    utils.ssh(himn, username, 'service', 'network', 'restart')
    utils.execute('rm', '-f', filename)


def get_mesh_info(astute, br_mesh):
    # private_network_range: 192.168.2.0/24
    mesh_nets = astute['network_scheme']['endpoints'][br_mesh]['IP'][0]
