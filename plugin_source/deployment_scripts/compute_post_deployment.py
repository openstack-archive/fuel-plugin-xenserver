...
AUTO_SCRIPT_TEMPLATE = 'fuel-xs-vxlan-template.sh'
...
def create_dom0_mesh_bridge(himn, username, dom0_bridge, mesh_info):
    ...
    if create_mesh_bridge:
        ...
        # Replace the template with correct value for mesh bridge
        utils.execute('rm', '-f', AUTO_SCRIPT)
        utils.execute('cp', AUTO_SCRIPT_TEMPLATE, AUTO_SCRIPT)
        utils.execute('sed', '-i', 's/DOM0_BRIDGE/%s/g' % dom0_bridge,
                      AUTO_SCRIPT)
        utils.execute('sed', '-i', 's/MESH_IP/%s/g' % mesh_info['ipaddr'],
                      AUTO_SCRIPT)
        utils.execute('sed', '-i',
                      's/MESH_NETMASK/%s/g' % mesh_info['netmask'],
                      AUTO_SCRIPT)
        utils.execute('sed', '-i',
                      's/MESH_BROADCAST/%s/g' % mesh_info['broadcast'],
                      AUTO_SCRIPT)
        utils.execute('sed', '-i', 's/CREATE_TAG/%s/g' % create_tag,
                      AUTO_SCRIPT)
        utils.execute('sed', '-i', 's/TAG/%s/g' % tag, AUTO_SCRIPT)
